...
public class TEST_03BaseAutomationLogic extends BaseAutomationLogic {
	...
	public ParameterSet execute(ParameterSet inputParameterSet) throws AutomationFailException{
		...
		if(globalParameterSet.getParameter("dsResult") != null){
			outputParameterSet.add(globalParameterSet.getParameter("dsResult"));
		}
	...
	private void initParameterSet(ParameterSet globalParameterSet) {
		...
		dsResult.addColumn("ROWINDEX", PlatformDataType.STRING, 255);
		dsResult.addColumn("STATUS", PlatformDataType.STRING, 255);
		dsResult.addColumn("RESULT", PlatformDataType.STRING, 255);
		dsResult.addColumn("ERROR", PlatformDataType.STRING, 255);

		globalParameterSet.add(dsResult);
	}
	...
	public void modify(ParameterSet globalParameterSet) throws AutomationFailException {//!findOffset_modify!
		...
		tmpParameterSet.add(globalParameterSet.getParameter("dsDeleteList"));

		ResultNameSet resultNameSet = new ResultNameSet();
		resultNameSet.add("RESULT0", "dsResult", null);
		...
		info.setSqlInsert("DELETE FROM EMPLOYEE\n WHERE\n EMP_ID = #dsDeleteList.EMP_ID#\n AND NAME = #dsDeleteList.NAME#\n AND DEPARTMENT = #dsDeleteList.DEPARTMENT#\n ");
		info.setSqlUpdate("");
		info.setSqlDelete("");
		info.setResultNameSet(resultNameSet);
		info.setParameterSet(tmpParameterSet);
		info.setLoopTargetDataSetName("dsDeleteList");
